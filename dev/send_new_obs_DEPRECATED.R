# FUNCTION DEPRECATED AS EXCLUSIION FLAGS CAN NOW BE POSTED DIRECTLY VIA RADIO
# AGENT REQUESTS - SO NO NEED TO PATCH THEM SEPERATELLY
...
#' @param additional_cols character vector, Names of additional columns in `post_dt`
...
send_new_obs <- function(post_dt, tm_id_col, additional_cols, api_base_url, token){
  
  ### ensure listed additional_cols are in data
  missing_additional_cols <- additional_cols %!in% names(post_dt)
  if(sum(missing_additional_cols) > 0){
    cli::cli_abort("Columns {.val {additional_cols[missing_additional_cols]}} not found in {.arg post_dt}.")
  }
  ...
  obs_excl_flags <- dplyr::right_join(
    ...
    dplyr::mutate(
      exclusion_flags = dplyr::if_else(cluster_status == "ACTIVE", 1311673391471656960, 0, missing = 0),
      er_obs_id = id,
      .keep = "unused"
    ) |> 
