...
match_sf_clusters <- function(masterclusters,
  ...
  if (!cluster_id_col %in% names(masterclusters) || !cluster_id_col %in% names(newclusters)) {
    cli::cli_abort("Cluster ID column '{cluster_id_col}' not found in both datasets.")
  }
  ...
  # Valid. Proceed
  cli::cli_inform("Matching {length(unique(newclusters[[cluster_id_col]]))} new clusters to {length(unique(masterclusters[[cluster_id_col]]))} master clusters.")
  cli::cli_inform("Matching criteria: {matching_criteria}, threshold: {matching_threshold}.")
  ...
  cli::cli_inform("Calculating centroids for old and new clusters based on matching criteria: {matching_criteria}.")
  ...
  if (matching_criteria == "gmedian") {
    master_centroids <- masterclusters |> 
      ...
      dplyr::summarise(geometry = calcGMedianSF(geometry), .groups = "drop")
  ...
  cli::cli_inform(
    "Found {nrow(matches)} matches, of which {nrow(double_matches)} are double-matches."
  )
  ...
  cli::cli_inform(
    "Found {nrow(unmatched_new_clusters)} unmatched new clusters. Adding to matchtable"
  )
  ...
  outdata <- matched_old_obs |> 
    ...
    dplyr::select(
      -any_of(c("XTEMPCLUSTER", "new_cluster"))
    )
  ...
  if (plot_results) {
    ...
    cli::cli_inform("Preparing output cluster-matching plots")
  ...
  cli::cli_alert_success(
    "Matched {nrow(final_matches)} clusters from old data to new data."
  )
